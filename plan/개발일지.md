...
할 것: 점검 전) **결석소명** 메인화면 최대한 맞추기, 뉴스 그라데이션으로 제목 확실하게 보이기, 전적검색 테이블 헤드 색 다르게, 테이블 바디 투명도 증가, 점검 중) 모드+캐릭+무기 증폭 백엔드 모델 수정, 패치 내용 적용, 엠마 데이터 추가 점검 후) api문서 읽고 적용방안 최대한 빠르게 생각해서 도입